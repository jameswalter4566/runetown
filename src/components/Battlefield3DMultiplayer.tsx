...
function CharacterModel({ faction, position, rotation, isMoving }: { 
  ...
  useEffect(() => {
    ...
    if (character instanceof SkeletonArcher) {
      console.log('SkeletonArcher detected in multiplayer, applying standard battlefield scale');
    }
...
interface Battlefield3DMultiplayerProps {
  player: Player;
  factions: Record<FactionType, Faction>;
  gameState: GameState;
}

const Battlefield3DMultiplayer: React.FC<Battlefield3DMultiplayerProps> = ({ player, factions, gameState }) => {
  ...
  const [arrows, setArrows] = useState<Array<{ id: string; position: THREE.Vector3; direction: THREE.Vector3; playerId?: string }>>([]);
  const [verticalLook, setVerticalLook] = useState(0);
  
  // Multiplayer hook
  const {
    gameState: multiplayerState,
    updateLocalPlayer,
    shootArrow: broadcastArrow,
    getOtherPlayers,
    isConnected
  } = useMultiplayer({
    roomId: `${currentRoom}-main`, // Different rooms for spawn and territory
    faction: player.faction,
    screenName: player.screenName,
    onPlayerJoin: (player) => {
      console.log('Player joined:', player.screenName);
    },
    onPlayerLeave: (playerId) => {
      console.log('Player left:', playerId);
    },
    onArrowShot: (arrowData) => {
      // Handle arrows from other players
      console.log('Received arrow from other player:', arrowData);
      const newArrow = {
        id: `arrow-${arrowData.playerId}-${arrowData.timestamp}`,
        position: new THREE.Vector3(arrowData.position.x, arrowData.position.y, arrowData.position.z),
        direction: new THREE.Vector3(arrowData.direction.x, arrowData.direction.y, arrowData.direction.z),
        playerId: arrowData.playerId
      };
  ...
  // Update multiplayer state when position/rotation changes
  useEffect(() => {
    const now = Date.now();
    if (now - lastMultiplayerUpdate.current < 33) return; // Throttle to 30 FPS
  ...
  }, [playerPosition, playerRotation, isMoving, player, updateLocalPlayer]);
  ...
  const shootArrow = useCallback(() => {
    ...
    // Broadcast arrow to other players
    console.log('Broadcasting arrow:', { position: arrowPosition, direction: arrowDirection });
    broadcastArrow({ position: arrowPosition, direction: arrowDirection });
  }, [inventory.arrows, playerPosition, playerRotation, verticalLook, broadcastArrow, multiplayerState.localPlayerId]);
    ...
    <div className="relative w-full h-screen">
      ...
      <div className="absolute top-20 left-4 bg-black bg-opacity-75 text-white p-4 rounded-lg z-10">
        ...
        <p className="text-sm mt-2">
          {isConnected ? (
            <span className="text-green-400">🟢 Online ({getOtherPlayers().length} players)</span>
          ) : (
      ...
      {showMap && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black bg-opacity-90 p-4 rounded-lg z-40">
          ...
          <div className="relative bg-gray-800 rounded" style={{ width: '600px', height: '600px' }}>
            ...
            {getOtherPlayers().map(otherPlayer => (
              <div
                key={otherPlayer.id}
                className="absolute w-2 h-2 bg-blue-400 rounded-full border border-white z-40"
                style={{
                  left: (otherPlayer.position.x * 0.6) + 300 - 4,
                  top: (otherPlayer.position.z * 0.6) + 300 - 4
                }}
                title={otherPlayer.screenName}
              />
            ))}
...
export default Battlefield3DMultiplayer;